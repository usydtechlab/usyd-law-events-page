...
fetch("data/data.json")
  ...
  .then(data => {
    ...
    eventDOMElements.forEach(element => {
      ...
      event.isTrending = generateTrendingEvents();
    ...
    let trendingEventBtn = document.getElementById("trendingFilterAnchor");
    let upcomingEventBtn = document.getElementById("upcomingEventsAnchor");

    trendingEventBtn.addEventListener("click", () => {
      events.forEach(event => {
        if (!event.isTrending) {
          event.parent.style.display = "none";
        }
        trendingEventBtn.classList += " active";
        upcomingEventBtn.classList = "nav-link";
      });
    });
    ...
    upcomingEventBtn.addEventListener("click", () => {
      ...
      upcomingEventBtn.classList += " active";
      trendingEventBtn.classList = "nav-link";
    });
    ...
    function generateTrendingEvents() {
      let randomNum = Math.random();
      if (randomNum > 0.4) {
        return true;
      } else {
    ...
    function handleCheckbox(event) {
      ...
      if (checkedList.length <= 0 || checkedList == []) {
        ...
        checkedList.forEach(element => {
          events.forEach(event => {
            if (
              event.description.toLowerCase().includes(element.toLowerCase()) ||
...
const generateEvents = data => {
  data.forEach(item => {
    ...
    contentCardBody.append(cardLocation);
